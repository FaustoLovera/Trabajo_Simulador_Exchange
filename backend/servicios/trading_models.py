...
def preparar_contexto_para_trading():
    """
    ...
    monedas_propias = {ticker: saldo for ticker, saldo in billetera.items() if saldo > 0}
    ...
    lista_monedas_propias_js = [
        {"ticker": ticker, "nombre": ticker} 
        for ticker in monedas_propias.keys()
    ]
